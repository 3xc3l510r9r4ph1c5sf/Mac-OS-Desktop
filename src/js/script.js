...
// Initialize spotlight as hidden
elements.spotlight_search.style.display = "none";
  ...
  if (navigator.getBattery) {
    navigator
      ...
      .then(function (battery) {
        number = Math.floor(battery.level * 100);
      ...
      .finally(() => {
        updateBatteryDisplay(number, batteryIsCharging);
      });
  } else {
    updateBatteryDisplay(number, batteryIsCharging);
  }
...
function updateBatteryDisplay(number, batteryIsCharging) {
  ...
  if (number <= 20) {
    ...
    elements.batteryIsChargingLogo.classList.add("is-charging-visible");
...
calculateBattery();
digi();
setInterval(digi, 1000); // Update clock every second